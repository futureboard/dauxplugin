/*
 * daux_host.cpp - A default host callback table for headless processing/scanning.
 *
 * Host-side C++ helper (daux_core). Provides a static daux_host_callbacks whose
 * callbacks are safe no-ops (logging goes to stderr). Real hosts (Futureboard,
 * DAUxHost in plugin mode) supply their own table that records automation,
 * forwards resize requests to the editor window, etc.
 */
